...
const SystemSettingsForm: React.FC<{ admin: Admin, onSave: (admin: Admin) => Promise<void> }> = ({ admin, onSave }) => {
    ...
    const handleDownloadBackup = async () => {
        if (!window.confirm("Download a live system snapshot? This file contains all your users, history, and wallet data.")) return;
        setIsBackingUp(true);
        try {
            ...
            a.href = url;
            a.download = `ABABA_DATA_BACKUP_${new Date().toISOString().split('T')[0]}.sqlite`;
            document.body.appendChild(a);
            a.click();
            a.remove();
        } catch (e) {
            alert("Backup failed. Please contact developer.");
        } finally {
    ...
    return (
        <div className="space-y-8 max-w-2xl mx-auto">
            <div className="bg-slate-800/50 p-6 rounded-lg border border-slate-700">
                <h3 className="text-xl font-bold text-white mb-6 uppercase tracking-widest border-b border-slate-700 pb-2 flex items-center gap-2">Admin Profile</h3>
                <form onSubmit={(e) => { e.preventDefault(); onSave({ ...admin, name: formData.name, avatarUrl: formData.avatarUrl, ledger: [], prizeRates: { oneDigitOpen: Number(formData.prizeRates.oneDigitOpen), oneDigitClose: Number(formData.prizeRates.oneDigitClose), twoDigit: Number(formData.prizeRates.twoDigit) } }); }} className="space-y-6">
                    ...
                    <button type="submit" className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-black py-3 rounded-md transition-all uppercase tracking-widest">Update Profile</button>
                </form>
            </div>
            ...
            <div className="bg-slate-800/50 p-6 rounded-lg border border-red-900/30">
                ...
                <p className="text-sm text-slate-400 mb-6 font-sans">The market is synchronized to reset automatically every day at 4:00 PM PKT. You should download a manual backup before performing any VPS maintenance.</p>
                <button 
                ...
                >
                    {isBackingUp ? (
                        <><div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin"></div><span>Generating Snapshot...</span></>
                    ) : (
                        ...
                        <span className="uppercase tracking-widest">Download Full System Backup (.sqlite)</span></>
                    )}
                </button>
                <div className="mt-4 flex justify-between text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em] px-1">
                    <span>Daily Reset Sync:</span>
    ...
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      {activeTab === 'dashboard' && summaryData && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-fade-in">
            <div className="bg-slate-800/50 p-6 rounded-lg border border-slate-700">
                <p className="text-xs text-slate-400 uppercase tracking-widest mb-1">System Liquidity</p>
            ...
            <div className="bg-slate-800/50 p-6 rounded-lg border border-slate-700">
             ...
             <div className="bg-slate-800/50 p-6 rounded-lg border border-slate-700">
                <p className="text-xs text-slate-400 uppercase tracking-widest mb-1">Prize Liability</p>
                <p className="text-2xl font-bold text-amber-400 font-mono">PKR {summaryData.totals.totalPayouts.toLocaleString()}</p>
            </div>
             <div className="bg-slate-800/50 p-6 rounded-lg border border-slate-700">
                <p className="text-xs text-slate-400 uppercase tracking-widest mb-1">Net Performance</p>
        ...
        <div className="bg-slate-800/50 rounded-lg overflow-hidden border border-slate-700">
            <table className="w-full text-left">
                <thead className="bg-slate-800/50">
                    <tr><th className="p-4 text-xs font-semibold text-slate-400 uppercase">Dealer</th><th className="p-4 text-xs font-semibold text-slate-400 uppercase text-right">Wallet</th><th className="p-4 text-xs font-semibold text-slate-400 uppercase text-center">Audit</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-800">
                    {props.dealers.map((d: any) => (
                        <tr key={d.id}>
                            <td className="p-4 text-white font-bold">{d.name} <span className="text-xs text-slate-500">({d.id})</span></td>
                            <td className="p-4 font-mono text-emerald-400 text-right">PKR {d.wallet.toLocaleString()}</td>
                            <td className="p-4 text-center">
                                <button onClick={() => setViewingLedgerId(d.id)} className="text-xs bg-slate-700 px-3 py-1 rounded text-cyan-400 uppercase font-bold hover:bg-slate-600 transition-all">Ledger</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
      ...
      {viewingLedgerId && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center p-4 z-50 animate-fade-in">
            <div className="bg-slate-900 border border-slate-700 w-full max-w-5xl rounded-lg overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-4 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
                    <h3 className="font-bold text-cyan-400 uppercase tracking-widest">Transaction Audit</h3>
                    <button onClick={() => setViewingLedgerId(null)} className="text-slate-400 hover:text-white">{Icons.close}</button>
                </div>
