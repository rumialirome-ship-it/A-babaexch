...
const verifySchema = () => {
    try {
        ...
        if (!stmt.get()) {
            console.error('--- [DATABASE] Schema missing. Ensure setup-database.js was run. ---');
            process.exit(1);
        }
    } catch (error) {
...
const findAccountForLogin = (loginId) => {
    // DEFINITIVE FIX: Prevent toLowerCase() crash
...
const declareWinnerForGame = (gameId, winningNumber) => {
    ...
    runInTransaction(() => {
        ...
        if (!game || game.winningNumber) throw new Error('Game not found or result already set.');
        if (game.name === 'AK') {
...
const createDealer = (d) => {
    if (db.prepare('SELECT id FROM dealers WHERE LOWER(id) = ?').get(d.id.toLowerCase())) throw new Error("ID already taken.");
...
const createUser = (u, dId, dep = 0) => {
    ...
    if (dep > 0) { addLedgerEntry(dId, 'DEALER', `Initial user funding`, dep, 0); addLedgerEntry(u.id, 'USER', `Funded by Dealer`, 0, dep); }
...
const placeBulkBets = (uId, gId, groups) => {
    ...
    runInTransaction(() => {
        ...
        addLedgerEntry(user.id, 'USER', `Bet placed`, requestTotal, 0);
        ...
        if (userComm > 0) addLedgerEntry(admin.id, 'ADMIN', `Comm payout`, userComm, 0);
        if (dComm > 0) { 
            addLedgerEntry(admin.id, 'ADMIN', `Override Comm`, dComm, 0); 
...
function resetAllGames() {
    try {
        ...
        console.error('--- [DATABASE] Daily boundary reached. Market reset executed. ---');
    } catch (e) {
