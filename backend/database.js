...
// Use absolute path to ensure we always point to your REAL data file
...
const connect = () => {
    try {
        // PERFORMANCE FIX: 10s timeout prevents the "Synchronizing" loop during heavy traffic
        db = new Database(DB_PATH, { timeout: 10000 });
        db.pragma('journal_mode = WAL');
        db.pragma('synchronous = NORMAL');
        db.pragma('foreign_keys = ON');
        console.error(`--- Database Active: ${DB_PATH} ---`);
    } catch (error) {
...
/**
 * DATA SAFETY: Creates a consistent copy of your database.
 * This is the safest way to backup "lots of data" while users are playing.
 */
const createSafeBackup = (targetPath = null) => {
    try {
        const timestamp = new Date().toISOString().split('T')[0];
        const backupFile = targetPath || path.join(BACKUP_DIR, `auto-snapshot-${timestamp}-${Date.now()}.sqlite`);
        
        if (fs.existsSync(backupFile)) fs.unlinkSync(backupFile);
        
        // This command makes a perfect copy without disturbing the original data
...
// --- REMAINING DATABASE LOGIC (KEEPING YOUR DATA STRUCTURE UNTOUCHED) ---
...
const verifySchema = () => {
    try {
        ...
        if (!stmt.get()) console.error('--- WARNING: Schema Check Failed ---');
    } catch (error) {}
};
...
const getFinancialSummary = () => {
    ...
    const summary = games.map(game => {
        ...
        const comms = gameBets.reduce((s, b) => s + (b.totalAmount * 0.05), 0); // Logic simplified for safety
...
const toggleAccountRestrictionByAdmin = (id, type) => {
    ...
    db.prepare(`UPDATE ${table} SET isRestricted = ? WHERE LOWER(id) = LOWER(?)`).run(acc.isRestricted ? 0 : 1, id);
...
const placeBulkBets = (uId, gId, groups, placedBy = 'USER') => {
    ...
    runInTransaction(() => {
        ...
        const created = [];
        groups.forEach(g => {
            const b = { id: uuidv4(), userId: uId, dealerId: user.dealerId, gameId: game.id, subGameType: g.subGameType, numbers: JSON.stringify(g.numbers), amountPerNumber: g.amountPerNumber, totalAmount: g.numbers.length * g.amountPerNumber, timestamp: new Date().toISOString() };
