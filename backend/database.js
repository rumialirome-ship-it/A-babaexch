...
function resetAllGames() {
    ...
    // It clears the current session's winning numbers and approval statuses
    // in the games table to provide a fresh interface for the next betting day.
    runInTransaction(() => {
        db.prepare('UPDATE games SET winningNumber = NULL, payoutsApproved = 0').run();
    });
    console.log('--- [DATABASE] Daily game reset executed. Winning numbers cleared for the new cycle. ---');
}
