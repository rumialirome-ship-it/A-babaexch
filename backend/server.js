...
function scheduleNextGameReset() {
    ...
    const delay = Math.max(60000, resetTime.getTime() - now.getTime()); // Ensure at least 1 min delay
    console.error(`--- Reset Scheduled: ${resetTime.toUTCString()} ---`);
...
app.post('/api/user/ai-lucky-pick', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const prompt = `Lucky numbers for "${gameType}" game. ${count} unique numbers. 1D: 0-9. 2D: 00-99. Numbers only, comma-separated.`;
