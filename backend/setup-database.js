...
function main() {
    ...
    if ((gamesCount === 0 || adminCount === 0) && fs.existsSync(SEED_DATA_PATH)) {
        console.log("Database empty. Seeding data from db.json...");
        ...
        // Seed Games
        const insertGame = db.prepare('INSERT OR IGNORE INTO games (id, name, drawTime) VALUES (?, ?, ?)');
        seedData.games.forEach(game => {
            insertGame.run(game.id, game.name, game.drawTime);
        });

        // Seed Dealers
        const insertDealer = db.prepare('INSERT OR IGNORE INTO dealers (id, name, password, area, contact, wallet, commissionRate, prizeRates, avatarUrl) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
        ...
        // Seed Users
        const insertUser = db.prepare('INSERT OR IGNORE INTO users (id, name, password, dealerId, area, contact, wallet, commissionRate, prizeRates, avatarUrl) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        ...
        console.log(`Seeding complete. ${seedData.games.length} games seeded.`);
    } else {
